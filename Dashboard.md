...
For the first exercise, I will be generating some suspicious activity on my windows machine. Afterwards, I will set up a dashboard on Wazuh to monitor and display relevant information about the activity.
...
2. The second element is table showing failed SSH authentication attempts to the connected Ubuntu machine. The table displays additional information such as the time of the authentication, the source ip address, and the source user.
...
The Dashboard proves to be a way to combine records from many different areas on the server and display them in an easily digestible way for users.
